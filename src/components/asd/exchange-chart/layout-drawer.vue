<template>
	<div class="layout-drawer">
		<SvgIcon
			name="bx-icon-multiple-layout"
			class="layout-icon icon relative inline-block color-text-1 calculate-icon cursor-pointer relative"
		...
		<a-drawer :width="380" :visible="visible" :closable="false" :header="false" :footer="false" @cancel="handleCancel">
            ...
            <SvgIcon
              ...
              :class="[
                'panel-icon',
                layoutStrategy === LayoutStrategy.STANDARD ? 'active' : '',
              ]"
...
const layoutStrategy = ref(store.getLayoutStrategy);
...
function handleLayoutChange(strategy: LayoutStrategy) {
	...
	store.setLayoutStrategy(strategy);
	visible.value = false;
}

function resetLayout() {
	store.resetLayout(true);
	visible.value = false;
}
</script>
